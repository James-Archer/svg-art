...
def draw_diagonal(x, y, x2, y2, color='black', thickness=2):
    ...
    return f'<line x1="{x}" y1="{y}" x2="{x2}" y2="{y2}" stroke="{color}" stroke-width="{thickness}" />'
    ...
    for row in range(0, height, step):
      for col in range(0, width, step):
        ...
        if random.choice([0, 1]) == 0:
        	#write the line
            output += draw_diagonal(col, row, col + step, row + step, color, thickness) + '\n'
        else:
        	#write the line
            output += draw_diagonal(col + step, row, col, row + step, color, thickness) + '\n'
    #write the footer
    footer = f'</svg>'
    output += footer
    return output
